...
class TestAnalyzeCommand:
    ...
    def test_analyze_with_level_filter(self, sample_log_file):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ["analyze", str(sample_log_file), "--level", "ERROR"])
    ...
    def test_analyze_with_match_filter(self, sample_log_file):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ["analyze", str(sample_log_file), "--match", "database"])
    ...
    def test_analyze_with_regex_filter(self, sample_log_file):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ["analyze", str(sample_log_file), "--regex", r"ERROR.*database"])
    ...
    def test_analyze_with_timestamp_range(self, sample_log_file):
        ...
        result = runner.invoke(cli, [
            "analyze", str(sample_log_file),
            "--from-ts", "2025-11-20 17:45:33",
            "--to-ts", "2025-11-20 17:45:35"
        ])
    ...
    def test_analyze_combined_filters(self, sample_log_file):
        ...
        assert result.exit_code == 0
        assert "ERROR Failed to connect to database" in result.output
        assert "CRITICAL System shutdown initiated" not in result.output  # doesn't match "Failed"
...
class TestStatsCommand:
    ...
    def test_stats_markdown_output(self, sample_log_file):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ["stats", str(sample_log_file), "--format", "markdown"])
    ...
    def test_stats_from_stdin(self, sample_log_lines):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ["stats", "--format", "json"], input="\n".join(sample_log_lines))
...
class TestTailCommand:
    ...
    def test_tail_from_start(self, tmp_path):
        ...
        log_file.write_text("2025-11-20 17:45:32 INFO Starting\n2025-11-20 17:45:33 ERROR Failed\n")
