...
from cli import DEFAULT_DATE_FORMAT, LogFilters, LogLine, build_filters
...
class TestLogFilters:
    ...
    def test_filters_from_timestamp(self):
        ...
        line_before = LogLine(
            "test.log", "Before", datetime(2025, 11, 20, 17, 45, 29), "INFO"
        )
        line_equal = LogLine(
            "test.log", "Equal", datetime(2025, 11, 20, 17, 45, 30), "INFO"
        )
        line_after = LogLine(
            "test.log", "After", datetime(2025, 11, 20, 17, 45, 31), "INFO"
        )
    ...
    def test_filters_timestamp_range(self):
        ...
        line_inside = LogLine(
            "test.log", "Inside", datetime(2025, 11, 20, 17, 45, 32), "INFO"
        )
        line_after = LogLine(
            "test.log", "After", datetime(2025, 11, 20, 17, 45, 36), "INFO"
        )
    ...
    def test_filters_combined_all_pass(self):
        ...
        to_ts = datetime(2025, 11, 20, 17, 45, 35)
        filters = LogFilters(
            from_ts=from_ts, to_ts=to_ts, levels=["ERROR"], match="database"
        )
...
class TestBuildFilters:
    ...
    def test_build_filters_from_timestamp(self):
        ...
        filters = build_filters(
            "2025-11-20 17:45:30", None, DEFAULT_DATE_FORMAT, [], None, None
        )
    ...
    def test_build_filters_to_timestamp(self):
        ...
        filters = build_filters(
            None, "2025-11-20 17:45:30", DEFAULT_DATE_FORMAT, [], None, None
        )
    ...
    def test_build_filters_levels(self):
        ...
        filters = build_filters(
            None, None, DEFAULT_DATE_FORMAT, ["ERROR", "WARN"], None, None
        )
    ...
    def test_build_filters_match(self):
        ...
        filters = build_filters(None, None, DEFAULT_DATE_FORMAT, [], "database", None)

        assert filters.match == "database"

    def test_build_filters_regex(self):
        """Test building filters with regex"""
        filters = build_filters(None, None, DEFAULT_DATE_FORMAT, [], None, r"error \d+")
    ...
    def test_build_filters_invalid_date_format(self):
        ...
        with pytest.raises(ValueError):
            build_filters("invalid-date", None, DEFAULT_DATE_FORMAT, [], None, None)

    def test_build_filters_invalid_regex(self):
        """Test building filters with invalid regex raises error"""
        with pytest.raises(re.error):
            build_filters(None, None, DEFAULT_DATE_FORMAT, [], None, r"[invalid(")
